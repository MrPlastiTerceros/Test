...
function verPostsUsuario(idUsuario) {
    ...
    for (var i = 0; i < listaPosts.length; i++) {
        ...
        var divchild= document.createElement("posteos");                 //crea div
        divchild.setAttribute("class", "post") ;                         //lo clasea(?   
        var posts = document.createTextNode("titulo post(temporal)");    //agrega el titulo del post     
        divcontenedorListaPost.appendChild(divchild);                    // inserta el div creado(divchild) en divContenedorListaPosts
